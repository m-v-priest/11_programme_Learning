...
....
:刘备: 蜀国皇帝刘玄德 //相当于kv键值对, 这行内容渲染后不可见

我是{刘备}  //会替换{}中的内容为之前定义的value值
....
...
https://asciidoc.org/asciidoc.css-embedded.html#X18
...
== ★ 插入 latex 代码 -> 单行用 stem:[] 包围 LaTex 语法, 多行则包在 "反斜杠(\)begin{align*} 和 "\end{align*}" 里面.
