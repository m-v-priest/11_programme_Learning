...
for i in pq(".cls_father").eq(0).children().items(): # children()是以"直接子节点"为最小单位.
    print(i.text())
    print('-' * 10)
...
....
pq = pyquery.PyQuery(html代码)
for i in pq(".cls_father").eq(0).children('p').items():
...
....
pq = pyquery.PyQuery(html代码)
for i in pq(".cls_father").eq(0).children('.cls_son').items():
